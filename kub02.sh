...
ExecStart=/usr/bin/etcd \
  ...
  --client-cert-auth \
  --initial-advertise-peer-urls https://172.31.41.254:2380 \
  --listen-peer-urls https://172.31.41.254:2380 \
  --listen-client-urls https://172.31.41.254:2379,http://127.0.0.1:2379 \
  --advertise-client-urls https://172.31.41.254:2379 \
...
echo ${MY_IP} && \
echo ${MY_HOSTNAME} && \
sed -i.bak "s/kub01/kub02/g" /etc/kubernetes/*.conf && \
sed -i.bak "s/172.31.27.31/172.31.41.254/g" /etc/kubernetes/*.conf && \
sed -i.bak "s/advertise-address=172.31.27.31/advertise-address=172.31.41.254/g" /etc/kubernetes/manifests/kube-apiserver.yaml && \
...
sed -i 's/172.31.27.31:6443/172.31.41.254:6443/g' /home/centos/.kube/config
